...
import { APP_CONFIG, PAPER_DIMENSIONS } from '@/config/app.config';
import type { PaperWidth } from '@/types';
import type { ExportResult, PrinterInfo, PrintJobOptions, PrintResult, SavePdfOptions } from '../types/electron';
...
/* ==========================================================================
   Printing in the main process (master spec §37, §68).

   - Documents arrive as complete, self-contained HTML (inline CSS, fonts as
     data: URIs) rendered by the React renderer.
   - Jobs print SILENTLY from a hidden, sandboxed window with a page exactly as
     wide as the roll paper and as tall as the content. The preview, printer
     and copies are chosen in the app's own print dialog. The operating-system
     print dialog is never used: it cannot preview these documents, and when
     opened for a hidden window it may never report back.
   - Every job settles — printed, or failed with a reason (including a
     timeout) — so the cashier's screen never waits forever.
   - "Save as PDF" renders the same page into a PDF file the user picks.
   ========================================================================== */
...
function printDirectory(): string {
  return path.join(app.getPath('temp'), 'restaurant-pos-print');
}
...
function assertPrintableHtml(html: unknown): asserts html is string {
  ...
  if (Buffer.byteLength(html, 'utf-8') > APP_CONFIG.maxPrintHtmlBytes) throw new Error('Print document is too large');
}
...
function paperPixels(paperWidth: PaperWidth): number {
  return Math.round(PAPER_DIMENSIONS[paperWidth].paperMm / MM_PER_CSS_PIXEL);
}

/**
 * Loads a document into a hidden, sandboxed window and runs `job` on it. The
 * window and its temp file are always removed afterwards.
 */
async function withDocumentWindow<T>(
  html: string,
  paperWidth: PaperWidth,
  title: string,
  job: (window: BrowserWindow) => Promise<T>,
): Promise<T> {
...
/**
 * Height of the document content (not the window), so the roll is cut right
 * after the last line instead of feeding blank paper.
 */
async function measureContentHeightMm(window: BrowserWindow): Promise<number> {
  const heightPx = (await window.webContents.executeJavaScript(
    '(() => { const body = document.body; if (!body) return 0; return Math.ceil(Math.max(body.getBoundingClientRect().height, body.scrollHeight)); })()',
    true,
  )) as number;
...
function renderPdf(html: string, paperWidth: PaperWidth, title: string): Promise<Buffer> {
  return withDocumentWindow(html, paperWidth, title, async (window) => {
    const heightMm = await measureContentHeightMm(window);
    return window.webContents.printToPDF({
      printBackground: true,
      margins: { top: 0, bottom: 0, left: 0, right: 0 },
      pageSize: {
        width: PAPER_DIMENSIONS[paperWidth].paperMm / 25.4,
...
/** Test mode (POS_PRINT_TO_PDF_DIR): every print job becomes a PDF file. */
async function saveJobToTestFolder(kind: string, html: string, options: PrintJobOptions): Promise<PrintResult> {
  ...
  const pdf = await renderPdf(html, options.paperWidth, options.title);
  await fs.promises.mkdir(directory, { recursive: true });
...
function printSilently(window: BrowserWindow, options: PrintJobOptions, heightMm: number): Promise<PrintResult> {
...
/**
 * Prints a document straight to the chosen printer (empty name = system
 * default). Never throws for printer problems: it reports what happened.
 */
export async function printDocument(kind: 'receipt' | 'kot', html: string, options: PrintJobOptions): Promise<PrintResult> {
  ...
  try {
    const result = await withDocumentWindow(html, options.paperWidth, options.title, async (window) =>
      printSilently(window, options, await measureContentHeightMm(window)),
    );
...
/** Where to write a PDF: chosen in a save dialog (or the test folder); null when cancelled. */
...
export async function savePdf(html: string, options: SavePdfOptions): Promise<ExportResult> {
  ...
  try {
    await writeFileAtomic(target, await renderPdf(html, options.paperWidth, fileName));
